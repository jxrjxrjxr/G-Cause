...
def bfs_2(
    transition_matrix, peak, entry_point, reach_end=True, max_path_length=None, verbose=False
):
    """ Backtrace breadth first search in the graph.
    ...
    while len(queue) > 0:
        ...
        if np.sum(transition_matrix[:, path[-1]]) == 0:
            ...
            if max_path_length is not None and len(path) >= max_path_length:
                ...
                curPeak = -1
                if len(path) > 1:
                    curPeak = peak["%s->%s"%(path[-1], path[-2])]
                for prev_node in range(transition_matrix.shape[0]):
                    if transition_matrix[prev_node, path[-1]] > 0.0 and (
                        prev_node not in path
                    ):
                        prePeak = peak["%s->%s"%(prev_node, path[-1])]
                        if curPeak != -1 and prePeak != -1 and curPeak <= prePeak:
                            continue
                        # extend the path
                        new_path = path + [prev_node]
                        queue.append(new_path)
...
def randwalk_2(
    ...
    matrix = transition_matrix
    new_transition_matrix = transition_matrix
    path_list = bfs_2(
        matrix,
        peak,
        entry_point,
